...
- гибкое расписание политик — фиксированный интервал (опционально ограниченный ежедневным окном бэкапа, окно может переходить через полночь) или 5-польное cron-выражение с IANA-таймзоной (с учётом DST); всё вычисление следующего запуска — на сервере, агент не затронут
...
- выполняет логические дампы PostgreSQL и MySQL, опционально сжимая дамп через zstd на лету (переключатель на уровне политики, по умолчанию включён; при восстановлении сжатые и несжатые артефакты определяются автоматически)
...
- редактор расписания в форме политики — режим интервала или cron с пресетами (ежедневно/еженедельно/ежемесячно), выбором таймзоны, опциональным окном бэкапа и живым предпросмотром следующих трёх запусков; списки политик показывают расписание в человекочитаемом виде («Daily at 03:00 (Europe/Moscow)»)
...
6. Задайте расписание: фиксированный интервал (опционально с ежедневным окном бэкапа) или cron-выражение с таймзоной — форма покажет предпросмотр следующих трёх запусков.
7. Сохраните.
