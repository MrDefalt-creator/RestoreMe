...
- страница каналов уведомлений (`/notifications`, только для admin) — добавление/редактирование/тест каналов Webhook, Telegram, Slack, Discord
...
Бэкенд пишет audit-записи на каждое критическое действие: создание / удаление / смена статуса / смена роли / сброс пароля пользователя, approve / reject / revoke агента, авто-отключение политики (`policy.auto_disabled`) и результаты доставки уведомлений (`notification.sent` / `notification.failed`). Admin-only `GET /api/audit-logs` возвращает paginated-список с username actor'а, join'нутым на сервере. Фронт показывает read-only страницу `/audit-log` (только для admin) с фильтром по action.
