...
- страница политик: создание, редактирование, тоггл, плюс бейдж «Auto-disabled» и повторное включение в один клик для политик, остановленных после серии провалов; в форме политики есть настройки ретенции (хранение по возрасту / количеству / суммарному размеру) и редактор расписания — интервал или cron с пресетами, выбором таймзоны, опциональным окном бэкапа и живым предпросмотром следующих трёх запусков
- страница задач: устойчивые лейблы, основанные на lookup'е агента/политики
- страница backups/artifacts: download, бейдж целостности артефакта и действие «Verify now»
- страница пользователей (управление доступом для admin)
- **страница каналов уведомлений** (только для admin) — создание/редактирование/тест каналов Webhook, Telegram, Slack, Discord с подпиской по типам событий; там же настройки расписания scrub целостности (только для admin)
...
- `POST /api/backupartifacts/{artifactId}/verify` (operator/admin — проверить целостность сейчас)
- `GET/PUT /api/integrity-settings` (только для admin — расписание scrub)
- `GET /api/users`
