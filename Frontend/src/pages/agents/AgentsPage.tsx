...
function formatPolicyType(policy: BackupPolicy, t: (key: string) => string) {
...
export function AgentsPage() {
  ...
  return (
    <div className="space-y-8">
      <SectionHeading
        eyebrow={t('Agents')}
        title={t('Registered machines')}
        description={t('Track machine identity, heartbeat freshness and the policies attached to every approved agent.')}
      />
      ...
      <Card className="space-y-4">
        <div className="relative max-w-md">
          ...
          <Input
            ...
            placeholder={t('Search by agent, machine or OS')}
            className="pl-10"
          />
        </div>
      </Card>
      ...
      {filteredAgents.length ? (
        <div className="grid gap-5 xl:grid-cols-[1.15fr_0.85fr]">
          ...
          {selectedAgent ? (
            <Card className="space-y-6">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.24em] text-accent-600">{t('Selected agent')}</p>
              ...
              <div className="grid grid-cols-2 gap-3">
                <MetaCard label={t('Version')} value={selectedAgent.version} />
                <MetaCard label={t('Created')} value={formatDateTime(selectedAgent.createdAt)} />
                  ...
                  selectedPolicies.map((policy) => (
                    <div key={policy.id} className="rounded-2xl border border-surface-200 bg-surface-50 px-4 py-3">
                      ...
                      <p className="mt-2 text-xs uppercase tracking-[0.18em] text-ink-800/60">
                        {formatPolicyType(policy, t)}
                      </p>
                      <p className="mt-2 text-sm text-ink-800">{formatPolicyTarget(policy)}</p>
                    </div>
                  ))
                ) : (
                  <EmptyState title={t('No policies')} description={t('The selected agent does not have backup policies yet.')} />
                )}
              </div>

              <div className="space-y-3">
                <h3 className="text-lg font-semibold text-ink-950">{t('Recent jobs')}</h3>
                {selectedJobs.length ? (
                  ...
                  <EmptyState title={t('No jobs yet')} description={t('This agent has not produced any backup history in the current dataset.')} />
                )}
              </div>
            </Card>
          ) : null}
        ...
        <EmptyState title={t('No agents found')} description={t('Try another search term or approve a pending machine first.')} />
      )}
    </div>
  )
}
